...
if const.LOCAL:
    app_utils, err_handler, utils, const, db = res
...
res, err = err_handler.error_wrapper(
    lambda: concurrent_update_gaps(local=True, csv_path=GAPS_CSV_PATH, run_dt=run_dt)) #concurrent_update_gaps(local=True) ###get_gb_gaps(df_serial.values, const.GB_GAPS_DATA_DIR, run_dt.isoformat()=run_dt.isoformat())
if err:
    logging.error(err)
    exit(3)
