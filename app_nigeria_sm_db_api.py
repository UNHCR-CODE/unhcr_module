...
# res, err = models.create_solarman_migration('Change device history key and remove id', eng)
# logger.info('Added created and updated columns to solarman schema tables')
# pass

#TODO: get stations upsert stations
stationList = [
        {
            "id": 63086751,
            ...
            "generationPower": 0.0,
            "lastUpdateTime": 1743211637.000000000,
            "contactPhone": None,
            "ownerName": None
        },
        {
            ...
            "generationPower": 0.0,
            "lastUpdateTime": 1743204324.000000000,
            "contactPhone": None,
            "ownerName": None
        },
        {
            ...
            "generationPower": 0.0,
            "lastUpdateTime": 1743211676.000000000,
            "contactPhone": None,
            "ownerName": None
        },
        {
            ...
            "networkStatus": "NORMAL",
            "generationPower": 0.0,
            "lastUpdateTime": 1743211699.000000000,
            "contactPhone": None,
            "ownerName": None
        }
    ]
# api_solarman.upsert_stations(stationList, eng=eng)
# pass
#!!!!!!!
# site_id_list =api_solarman.db_all_site_ids(eng)
# for site in site_id_list:
#     print(site)
#     api_solarman.api_get_devices(site, eng=eng)
# pass
...
#         key = next(iter(site))
#         print(key, site[key])
#         res = api_solarman.get_station_daily_data(id=site[key], start_date=start_dt, end_date=end_dt, type=2, eng=eng)
#         pass
# pass
#!!!!!!!!! STATION DAILY DATA

#!!!! initial load of device history TODO: make it general and prod ready
# sites = api_solarman.db_all_site_ids(eng)
# for site in sites:
#     res, err = models.db_update_device_history(site, eng)
#     pass
...
inverters_sn = api_solarman.db_get_inverter_sns(eng)
...
def process_chunk(chunk, date, engine):
    ...
    for sn in chunk:
        ...
        res, err = api_solarman.get_inverter_data(sn=sn, start_date=date, type=1, eng=engine)
        end_time = time.time()
...
while x > 0:
    ...
    with ThreadPoolExecutor(max_workers=num_parts) as executor:
        results = list(
            executor.map(lambda chunk: process_chunk(chunk, start_dt, eng), chunks)
        )
...
res, err = db.sql_execute(f"with x as (select device_sn, min(ts) ts FROM solarman.inverter_data group by 1)select max(ts)::date from x;", eng)
if err:
    logger.error(err)
    exit(9)
start_dt = res[0][0]
while 1 == 1:
    for sn in inverters_sn:
        ...
        start_time = time.time()
        res, err = api_solarman.get_inverter_data(sn=sn, start_date=start_dt, type=1, eng=eng)
        end_time = time.time()
        print(f'Execution time: {((end_time - start_time)/60):.2f} minutes')
        pass
    start_dt = start_dt - timedelta(days=1)
pass
# api_solarman.insert_inverter_data(data, eng=None)
# pass
